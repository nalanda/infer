...
variables. See
[`?gss`](https://infer.tidymodels.org/dev/reference/gss.md) for more
...
The [`specify()`](https://infer.tidymodels.org/dev/reference/specify.md)
...
[`specify()`](https://infer.tidymodels.org/dev/reference/specify.md)
just looks like it selects off the columns in the dataframe that you’ve
...
on the significance level $`\alpha`$ you decided on *before* you ran
this analysis. If you had set $`\alpha = .05`$, then this difference
would be statistically significant, but if you had set $`\alpha = .01`$,
...
significant at the confidence level $`\alpha = .05`$. To see this
interval represented visually, we can use the
...
[`calculate()`](https://infer.tidymodels.org/dev/reference/calculate.md)
with [`assume()`](https://infer.tidymodels.org/dev/reference/assume.md).
For example, to calculate the observed $`t`$ statistic (a standardized
mean):
...
When visualized, the $`t`$ distribution will be recentered and rescaled
to align with the scale of the observed data.
...
fitting. Rather than
[`calculate()`](https://infer.tidymodels.org/dev/reference/calculate.md)ing
...
just switch out
[`calculate()`](https://infer.tidymodels.org/dev/reference/calculate.md)
for [`fit()`](https://generics.r-lib.org/reference/fit.html) in your
