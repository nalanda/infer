...
The 1-sample $`t`$-test can be used to test whether a sample of
continuous data could have plausibly come from a population with a
specified mean.
...
demonstrating how to carry out a theory-based $`t`$-test with the
package.
...
function, `t_test`, to carry out 1-sample $`t`$-tests on tidy data.
Rather than using randomization, the wrappers carry out the theory-based
