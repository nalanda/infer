...
- x:
  ...
  For theory-based inference, output of
...
output of
[`assume()`](https://infer.tidymodels.org/dev/reference/assume.md) to
the `x` argument) require that the point estimate lies on the scale of
the data. The distribution defined in
[`assume()`](https://infer.tidymodels.org/dev/reference/assume.md) will
be recentered and rescaled to align with the point estimate, as can be
